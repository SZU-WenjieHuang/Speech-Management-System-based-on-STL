...
void SpeechManager::speechContest()
{
	...
	//multimap，在创建的时候就需要提供一个仿函数greater<int>来表示排序规则，这里是降序排列，默认是升序
	...
	for (vector<int>::iterator it = v_Src.begin(); it != v_Src.end(); it++)
	{
		...
		//评委打分
		deque<double>d; // deque用于记录分数 方便去掉最高分和最低分
		for (int i = 0; i < 10; i++)
		{
...
void SpeechManager::loadRecord()
{
	...
	while (ifs >> data)
	{
		...
		while (true)
		{
			...
			string tmp = data.substr(start, pos - start); //找到了,进行分割 参数1 起始位置，参数2 截取长度 这个操作相当于把逗号去掉了。
			v.push_back(tmp);
			start = pos + 1;
		}
